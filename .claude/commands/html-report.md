...
├──────────────────────────────────────────────  ┤
│  Applications  [Status ▾] [Sector ▾] [Job Board ▾] [🔍 ...]│  ← table with filters
...
- **Table:**
  ...
  - Client-side filter: a text search input filters rows across company + role + sector; the status, sector, and job board (multi-select / quick chips for Indeed, Jobs Canada / Job Bank, LinkedIn, Talent.com, Eluta) filters operate independently; all combine (AND)
